...
if_use_pretrained_weights =   True            # Set to True if using pretrained model
pretrained_model_dir = '/homeappl/home/wangshan/deep_clustering/net9'
pretrained_weights = os.path.join(pretrained_model_dir, 'best_weight_net9_100.hdf5')
...
if if_use_pretrained_weights:
   print('------------------------------------ pretrained model weights are being used from the directory:', pretrained_model_dir)
   model.load_weights(pretrained_weights)
   

checkpoint = ModelCheckpoint('./net9/best_weight_net9_200_basedon_100.hdf5', monitor='val_loss', verbose=1, save_best_only=True, mode='min')
...
his = model.fit([splitted_mix_spec_tr,splitted_VAD],Y_tr, validation_data=([splitted_mix_spec_cv,splitted_VAD_cv],Y_cv),batch_size=128, epochs=100 ,callbacks=callbacks_list)
...
model.save('./net9/net9_model_200_basedon_100.h5')
model.save_weights('./net9/ending_weight_net9_200_basedon_100.h5')
...
plt.savefig('./net9/loss-vs-val_loss_net9_200_basedon_100')
